# King of Racing

King of Racing adalah game **Action–Sports racing 1v1** dengan sistem **skill**, **leveling**, dan **upgrade customization**. Game ini menggabungkan mekanik balap cepat dengan narasi yang emosional tentang perjalanan, ambisi, dan keberanian untuk menghadapi masa lalu.

---

## 1. Informasi Umum

### Genre
Action – Sports (1v1 Competitive Racing)

### Target Audiens
Remaja & Pemuda usia **13+**

### Platform Rilis
- **PC (Windows)**

---

## 2. Core Mechanics & Gameplay

### Core Mechanics

| Fitur | Deskripsi |
|-------|-----------|
| **1v1 Cars Battle** | Duel kecepatan, trick, dan manuver untuk mendapatkan skor tertinggi. |
| **Trick & Skill System** | Pemain dapat melakukan kombinasi manuver untuk bonus poin. |
| **Leveling System** | Setiap race memberi EXP untuk membuka fitur baru & cerita lanjutan. |
| **Upgrade Equipment** | Koin dapat digunakan untuk meningkatkan speed, roda, turbo, dan efek aura. |
| **Story Progression** | Setiap kemenangan membuka chapter baru hingga melawan boss final. |
| **Lobby & Fair Start** | Sistem memastikan fairness saat race dimulai. |

### Gameplay Overview

- Kontrol menggunakan **W A S D**.
- Race berlangsung di jalan raya
- Pemain memperoleh poin dari:
  - Kecepatan
  - Trick
  - Manuver menghindari tantangan
  - Drift dan turbo timing
- Kemenangan membuka chapter & skill baru.
- Kekalahan → replay stage.
- Narasi mengikuti perjalanan **Cecep & Ojan** hingga menghadapi raja balap terakhir.

---

## 3. Story / Narrative

### Premis
Cecep (40 tahun) pernah mencintai balapan, namun trauma kecelakaan masa muda membuatnya meninggalkan dunia itu. Saat pindah rumah, ia kembali menemukan mobil lamanya—dan sejarah mulai terulang.

### Konflik
Cecep ingin membuang mobil itu, tetapi **Ojan**, pemuda pendiam, meminta mesinnya.

### Perkembangan
Tanpa sepengetahuan Cecep, Ojan memakai engine itu untuk bertarung melawan pembalap terhebat kota—membangkitkan sisi muda Cecep.

### Klimaks
Engine rusak—membangkitkan rasa penyesalan dalam diri Cecep.

### Resolusi
Cecep akhirnya berdamai dengan masa lalu dan kembali mengejar impiannya.

---

## 4. Visual Style & Audio

### Visual Style

- **Tema:** kota malam
- **Art Direction:** Semi-realistic stylized
- **Dominasi warna:** Biru, ungu neon, cyan

#### Environment
- Gedung tinggi 
- Jalan raya 
- Lampu jalanan kota & billboard LED

#### Character Design
| Karakter | Deskripsi |
|----------|-----------|
| **Cecep** | Pria 40-an, canggung namun hangat. |
| **Ojan** | Remaja pendiam, hoodie, earphone, ambisius. |
| **Cinema & Bawahan** | Karakter intimidatif dan gagah. |

#### Cars & UI
- Mobil dengan efek speed trail, turbo sparks, aura saat upgrade.
- UI modern, bold, kompetitif.

---

## Mekanik Kontrol

| Input | Fungsi |
|-------|--------|
| W | Maju |
| S | Mundur |
| A | Kiri |
| D | Kanan |

---

## Tools & Software

| Kategori | Tools | Fungsi |
|----------|-------|--------|
| Game Engine | Unity | Platform pengembangan game |
| 3D Assets | Mixamo | Animasi & asset model |
| UI Design | Figma | Mendesain tampilan UI/UX |
| Programming | Visual Studio | Scripting C# |

---

## Komponen Sistem

| Komponen | Fungsi |
|---------|--------|
| Player Controller | Mengatur karakter & kendaraan |
| NPC System | Pergerakan & behavior lawan |
| Event Manager | Mengontrol cutscene & trigger story |
| Audio Manager | Manajemen efek suara & musik |
| UI System | Subtitle, score, HUD |
| Scene Manager | Loading & transisi scene |

---

## Status

> **Stage:** Concept + Pre-production  
> Dokumentasi, prototyping mekanik dasar, & storyboard naratif.

---

## Credits

| Role | Member |
|------|--------|
| Project Manager | Ghira Jilan Zhafira |
| Game Designer | Zharira Hendhis Zahran |
| Programmer | Cinema Zaidan Zidna Fann |
| Artist & UI/UX | Hauzan Mahfudz |


---

### 📌 License

Game dan dokumentasi ini adalah bagian dari proyek internal tim dan tidak diperbolehkan dijual secara terpisah.

---

🚗🔥 **"Race Fast. Break Limits. Rewrite Your Past."**

